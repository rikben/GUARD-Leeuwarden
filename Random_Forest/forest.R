...
prepare_training_data <- function(image_csv_paths, parcel_csv_paths) {
  ...
  for (i in seq_along(image_csv_paths)) {
    ...
    numeric_cols <- grep("_mean$|_sd$", names(raw_data), value = TRUE)
    ...
    is_image_discarded <- toupper(trimws(as.character(raw_data$discarded))) %in% c("TRUE", "YES")
    
    year_data <- raw_data %>%
      filter(!parcel_id %in% discarded_parcels) %>%
      filter(!is_image_discarded[row_number()]) %>%
...
fit_rf_models <- function(train_set, optimal_trees = 500) {
  ...
  # Drop glyphosate from predictors — it's a label, not a spectral measurement
  train_boot <- train_boot %>% select(-glyphosate)
  
  list(
    # probability = TRUE: predict() returns a matrix of class probabilities
    # instead of a single label — required for the per-image confidence column
    model      = ranger(class_label ~ ., data = train_boot,
                        num.trees = optimal_trees,
                        importance = "permutation",
                        probability = TRUE),
    model_gini = ranger(class_label ~ ., data = train_boot,
                        num.trees = optimal_trees,
                        importance = "impurity",
                        probability = TRUE)
  )
...
run_cross_validation <- function(train_data, optimal_trees = 500, n_folds = 5, seed = 42) {
  ...
  predictor_cols <- grep("_mean$|_sd$", names(train_data), value = TRUE)
...
evaluate_rf_model <- function(rf_models, test_set) {
  ...
  model_gini <- rf_models$model_gini
  
  predictor_cols <- grep("_mean$|_sd$", names(test_set), value = TRUE)
  ...
  cat("\n===== Per-Class Stats =====\n")
  print(round(cm$byClass[, c("Sensitivity", "Specificity", "Pos Pred Value")], 3))
  
  cat("\n===== Variable Importance: Permutation =====\n")
  print(sort(ranger::importance(model), decreasing = TRUE))
  
  cat("\n===== Variable Importance: Gini (impurity) =====\n")
  print(sort(ranger::importance(model_gini), decreasing = TRUE))
...
predictor_cols <- grep("_mean$|_sd$", names(data_both), value = TRUE)
